    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="card-title">Acknowledged Alerts</h5>
                    <div class="card-subtitle">
                        Showing acknowledged alerts from the last 30 days by default
                    </div>
                </div>
        ...
        <div class="card-body">
            ...
            <div class="mb-4 row g-3 align-items-end">
                ...
                <div class="col-md-4">
                    <select wire:model.live="alarmCause" id="alarmCause" class="form-control select2 form-select"
                        placeholder="Select Alarm Cause">
                        <option value="">Cause</option>
                        @foreach ($alarmCauses as $cause)
                            <option value="{{ $cause }}">{{ $cause }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    <select wire:model.live="acknowledgePerson" id="acknowledgePerson"
                        class="form-control select2 form-select" placeholder="Select Acknowledge Person">
                        <option value="">Acknowledge Person</option>
                        @foreach ($acknowledgers as $person)
                    ...
                    <tbody>
                        @forelse($alerts as $alert)
                            <tr>
                                ...
                                <td>{{ $alert->acknowledged_at ? $alert->acknowledged_at->format('Y-m-d H:i:s') : 'N/A' }}
                                </td>
                                <td>{{ $alert->acknowledgedByUser->name ?? 'N/A' }}</td>
                                <td>{{ $alert->alarm_cause ?? 'N/A' }}</td>
                                <td>{{ $alert->notes ?? 'N/A' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="11" class="text-center">No acknowledged alerts found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <script>
            function initSelect2() {
                ...
                $('.form-select').each(function() {
                    ...
                    $this.select2({
                        placeholder: $this.attr('placeholder') || '-- Silahkan Pilih --',
                        allowClear: true,
                        width: '100%',
                ...
                $('#alarmCause').on('change.select2', function(e) {
                    var data = $(this).val();
                    @this.set('alarmCause', data);
                });

                $('#acknowledgePerson').on('change.select2', function(e) {
                    var data = $(this).val();
                    @this.set('acknowledgePerson', data);
                });
