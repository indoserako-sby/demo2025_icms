...
class AcknowledgedAlarmsExport implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithTitle, WithStyles, WithEvents
{
    protected $area;
    protected $group;
    protected $asset;
    ...
    public function __construct($area, $group, $asset, $parameter, $alertType, $alarmCause, $acknowledgePerson, $startDate, $endDate)
    {
        $this->area = $area;
        $this->group = $group;
        $this->asset = $asset;
    ...
    public function map($alert): array
    {
        ...
        return [
            ...
            $alert->acknowledged_at ? $alert->acknowledged_at->format('Y-m-d H:i:s') : 'N/A',
            $alert->acknowledgedByUser->name ?? 'N/A',
            $alert->alarm_cause ?? 'N/A',
            $alert->notes ?? 'N/A',
        ];
    }
